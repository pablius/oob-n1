...
class ADODB_ado extends ADOConnection {
	...
	var $_servicior_type = 3; // 3=adOpenStatic,0=adOpenForwardOnly,1=adOpenKeyset,2=adOpenDynamic
	var $_servicior_location = 3; // 2=adUseServer, 3 = adUseClient;
	var $_lock_type = -1;
	...
	function _connect($argHostname, $argUsername, $argPassword, $argProvider= 'MSDASQL')
	{
		...
		$dbc->ServiciorLocation = $this->_servicior_location;
		return  $dbc->State > 0;
		} catch (exception $e) {
		}
...
class ADORecordSet_ado extends ADORecordSet {	
	...
	function _initrs()
	{
	 ...
	 // should only be used to move forward as we normally use forward-only serviciors
	function _seek($row)
	{
	   $rs = $this->_queryID; 
