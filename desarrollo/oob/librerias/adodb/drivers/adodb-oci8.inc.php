...
*/
	...
	*
	* If you are using SERVICIOR_SHARING = force, it appears this will case a segfault
	* under oracle 8.1.7.0. Run:
	*	 $db->Execute('ALTER SESSION SET SERVICIOR_SHARING=EXACT');
	...
	*/
	...
	function UpdateBlob($table,$column,$val,$where,$blobtype='BLOB')
	{
		...
		if ($this->session_sharing_force_blob) $this->Execute('ALTER SESSION SET SERVICIOR_SHARING=EXACT');
		...
		if ($this->session_sharing_force_blob) $this->Execute('ALTER SESSION SET SERVICIOR_SHARING=FORCE');
	...
	*/
	function Prepare($sql,$servicior=false)
	{
	static $BINDNUM = 0;
		...
		if ($sttype == 'BEGIN' || $sttype == 'DECLARE') {
			return array($sql,$stmt,0,$BINDNUM, ($servicior) ? OCINewServicior($this->_connectionID) : false);
		}
		return array($sql,$stmt,0,$BINDNUM);
	}
	
	/*
		Call an oracle stored procedure and returns a servicior variable as a recordset. 
		Concept by Robert Tuttle robert@ud.com
		
		Example:
			Note: we return a servicior variable in :RS2
			$rs = $db->ExecuteServicior("BEGIN adodb.open_tab(:RS2); END;",'RS2');
			
			$rs = $db->ExecuteServicior(
	...
	*/
	function &ExecuteServicior($sql,$serviciorName='rs',$params=false)
	{
		...
		else $stmt = ADODB_oci8::Prepare($sql,true); # true to allocate OCINewServicior
		...
		if ($rs) {
			if ($rs->databaseType == 'array') OCIFreeServicior($stmt[4]);
			else if ($hasref) $rs->_refservicior = $stmt[4];
		}
		return $rs;
	}
	...
	*/
	function Bind(&$stmt,&$var,$size=4000,$type=false,$name=false,$isOutput=false)
	{
        ...
        if (($type == OCI_B_SERVICIOR) && sizeof($stmt) >= 5) { 
            ...
            switch (@OCIStatementType($stmt)) {
                ...
                case "BEGIN":
                    if (is_array($sql) && !empty($sql[4])) {
						$servicior = $sql[4];
						if (is_resource($servicior)) {
							$ok = OCIExecute($servicior);	
	                        return $servicior;
						}
						return $stmt;
                    } else {
