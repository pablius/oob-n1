...
class ADODB_informix72 extends ADOConnection {
	...
	var $serviciorType = IFX_SCROLL; // IFX_SCROLL or IFX_HOLD or 0
...
*/
	...
	global $ADODB_COUNTRECS;
	  ...
	  // In case of select statement, we use a scroll servicior in order
	  // to be able to call "move", or "movefirst" statements
	  if (!$ADODB_COUNTRECS && preg_match("/^\s*select/is", $sql)) {
		 if ($inputarr) {
			$this->lastQuery = ifx_query($sql,$this->_connectionID, $this->serviciorType, $tab);
		 }
		 else {
			$this->lastQuery = ifx_query($sql,$this->_connectionID, $this->serviciorType);
		 }
	  }
	  else {
		 if ($inputarr) {
