	...
	function letra(){
		...
		if(strlen($cnumero)>15){
			...
			if($this->mayusculas){//mayusculas
				$texto=strtoupper($this->prefijo.$this->moneda." ".$texto." CON ".substr($cnumero,-2)."/100 ".$this->sufijo);
			}else{//minusculas
				$texto=strtolower($this->prefijo.$this->moneda." ".$texto." CON ".substr($cnumero,-2)."/100 ".$this->sufijo);	
			}
		}
		return $texto;
