...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
	...
	/**
	 * @cfg {JsonStore} Aca se le pueden pasar archivos que ya esten cargados para que se muestren en el panel
	 */
	,savedfiles : false
	
	/**
	 * @cfg Aca se le pasa la direccion que de llama para borrar archivos, el js pasa un array con los ids de los archivos
	 */
	,deletedir : 'delete.php'
	/**
	 * @cfg para pasar algun parametro 
	 */
	,param : ''
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
		...
		this.uploader.baseParams.param = this.param;
	...
	,onRender:function() {
		...
		var count = 0;
		if( this.savedfiles ){ 
			this.savedfiles.each(function(c){
			count++;
					var lock = false;
					if( c.data.locked ){
						lock = true;
		...
		if( count  == 0){
		this.removeAllBtn.setDisabled(true);
		}
	...
	,getQtip:function(values) {
		...
		switch(values.state) {
			...
			case 'done':
			case 'saved':				
				qtip = String.format('File <b>{0}</b> is saved', values.fileName);			
				if( !values.locked ){				
				qtip += '<br>' + this.clickRemoveText;
				}
			break;
	...
	,onViewClick:function(view, index, node, e) {
		...
		var t = e.getTarget('div:any(.ux-up-icon-queued|.ux-up-icon-failed|.ux-up-icon-stopped)');
		if(t) {
			this.onRemoveFile(this.store.getAt(index));
		}

		var t = e.getTarget('div:any(.ux-up-icon-uploading)');	
		if(t){
			this.onRemoveFile(this.store.getAt(index));	
		}
			
		var t = e.getTarget('div:any(.ux-up-icon-saved|.ux-up-icon-done)');		
		if(t){
			var record = this.store.getAt(index);
			
			if( !record.get('locked') ){
			
			Ext.MessageBox.confirm('Emporika', 'Esta seguro que desea eliminar este archivo?' , 
			function(btn){
				if( btn == 'yes' ){	
			...
			cnx.request({url: this.deletedir,
			method : 'POST',
			scope : this,
			...
			success: function(responseObject){															
					...
					if( json.success == true ){
						...
						Ext.MessageBox.alert('Emporika', 'No se logro eliminar el archivo');
					}
