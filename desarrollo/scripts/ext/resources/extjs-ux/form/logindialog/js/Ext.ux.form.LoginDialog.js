...
Ext.ux.form.LoginDialog = function (config) {
    ...
    var css = ".ux-auth-header-icon {background: url('" + this.basePath + "/small/locked.gif') 0 4px no-repeat !important;}"
            + ".ux-auth-header {background:transparent url('" + this.basePath + "/large/lock.gif') no-repeat center right;padding:10px;padding-right:45px;font-weight:bold;}"
            + ".ux-auth-login {background-image: url('" + this.basePath + "/medium/key.gif') !important}"
            + ".ux-auth-close {background-image: url('" + this.basePath + "/medium/close.gif') !important}"
    ...
    this._formPanel = new Ext.form.FormPanel ({
        ...
        items : [{
            ...
            listeners: {
                render: function() {
                    Ext.get(Ext.DomQuery.select('#x-form-el-' + this._rememberMeId + ' input')).set({qtip:'This is not recommended for shared computers.'});
                },
                scope: this
            }
        }]
