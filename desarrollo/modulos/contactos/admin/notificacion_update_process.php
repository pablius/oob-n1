...
//CREO EL OBJETO
$notificacion = new contactos_notificacion($id);
...
//PASO LOS DATOS DEL FORMULARIO A VARIABLES 
	...
	if( $notificacion->store() ){
			...
			if( isset($_POST['contactos']) ){					
							...
							$filtros[] = array("field"=>"notificacion","type"=>"list","value"=>$notificacion->id() );
...
if ( $errores = $notificacion->error()->getErrors() )
{
   ...
   $error_codes = array();
   $error_codes['NO_TITULO'] = array( "id"=>"txt_titulo" , "msg"=>"Debe seleccionar un cliente" );
   $error_codes['NO_MENSAJE'] = array( "id"=>"txt_mensaje" , "msg"=>"Debe seleccionar la repetici&oacute;n" );
   ...
   $error_codes['NO_ISCOMUNICACION'] = array( "id"=>"fecha" , "msg"=>"Debe seleccionar si es comunicacion" );    
   $error_codes['NO_SENDMAIL'] = array( "id"=>"fecha" , "msg"=>"Debe seleccionar si es comunicacion" );    
