...
$clase_selected = 1; //cliente
	
	if( isset( $_POST['clase_selected'] ) )
	{
		$clase_selected = $_POST['clase_selected'];
	}
...
//FORMULARIO
$frm_contacto = new PhpExt_Form_FormPanel();
$frm_contacto->setFrame(true)
...
$frm_contacto->attachListener( "beforerender", new PhpExt_Listener( PhpExt_Javascript::functionDef( null, $form_render , array( "form" ) )) );			   		   	 				  			   
$frm_contacto->setEnableKeyEvents(true);				 
...
$txt_diaspago =  PhpExt_Form_NumberField::createNumberField( "txt_diaspago", "D&iacute;as Pago" )
				  ->setMsgTarget(PhpExt_Form_FormPanel::MSG_TARGET_SIDE)			   
			     ->setWidth($field_width)
				 ->setValue( $ari->module->config()->get('diaspago', 'contacto') );								   
...
$fieldset_usuarios->setHeight(300)					  
			      ->setTitle('Usuarios');	
...
$opt_automaticamente = new PhpExt_Form_Radio();
$opt_automaticamente->setFieldLabel("Crear Automaticamente")
					->setValue("automaticamente")
					->setName("opt-usuario");	
$opt_automaticamente->setChecked(true);							

$fieldset_usuarios->addItem($opt_automaticamente);					
...
//CAMPOS DEL STORE	  
$store_medioscontacto->addField(new PhpExt_Data_FieldConfigObject("direccion"));
$store_medioscontacto->addField(new PhpExt_Data_FieldConfigObject("tipo"));
$store_medioscontacto->addField(new PhpExt_Data_FieldConfigObject("id_tipo"));
$store_medioscontacto->addField(new PhpExt_Data_FieldConfigObject("prefix"));
...
panel.unhideTabStripItem(0);
var clase = '1' ;
var paneles = panel.findBy(function(c){ return (c.xtype == 'panel') });
...
while( i < paneles.length ){
	...
	if(!entro){
		panel.hideTabStripItem(paneles[i]);
	}
...
$tab_infoadicional->setEnableKeyEvents(true);
$tab_infoadicional->attachListener( "render", new PhpExt_Listener( PhpExt_Javascript::functionDef( null, $tab_render , array( "panel" ) )) );			   		   	 									   	
...
$frm_contacto->addItem( $tab_panel );		
...
function(){
	...
	var u = (radio[0].getValue())?'existente':((radio[1].getValue())?'nuevo':'automatico');
	...
	var new_tab_dir = '';
	if( activetabid == '' ){	
		new_tab_dir = '/contactos/contacto/view';	
	}
	...
	formulario.submit({      
				 ...
				 success_msg : 'Contacto creado correctamente',
			 grid_reload_id  : '{$grid_id}',
				 new_tab_dir : new_tab_dir,
			   load_tab_here : true,
			   new_tab_title : 'Detalle de Contacto',
new_tab_pass_response_params : { id : 'id'},
		 success_after_click : function(form,action){
					...
					if( combo ){
						...
						if(activetab){
							panel_tabs.setActiveTab(activetab);
						}
					
						combo.store.load();	
						if(hidden){
							hidden.setValue(obj.id);						
						}
						combo.setValue(obj.name);						
...
$frm_contacto->addButton( $send_button );		
			 
//RESULTADO
$obj_comunication = new OOB_ext_comunication();
$obj_comunication->set_data( $frm_contacto->getJavascript( false, "contenido" ) );
